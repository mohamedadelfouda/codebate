...
const PROVIDER_GLYPHS = {
  // Brand logos supplied by the user. Codex keeps its own gradient, but its shipped `<style>.a{fill:…}` +
  // class="a" was replaced with a direct fill (a global `.a` rule would bleed onto the rest of the page) and
  // its gradient ids are namespaced so repeated avatars don't clash. Cursor is monochrome (currentColor), so
  // it takes the avatar's brand tint.
