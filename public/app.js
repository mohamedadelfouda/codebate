...
const PROVIDER_GLYPHS = {
  ...
  // direct fill (a global `.a` rule would bleed onto the rest of the page), and its gradient was flattened to
  // a single objectBoundingBox linearGradient — the original's `href`-template + userSpaceOnUse matrix didn't
  // paint once inlined via innerHTML. Cursor is monochrome (currentColor), so it takes the avatar's brand tint.
