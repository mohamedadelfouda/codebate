...
test("scenario · RED orchestration: changing ledger conflict resets the degraded persistence streak", {
  skip: "orchestrator does not yet track a ledger-conflict signature — unskip with degraded_ledger_conflict",
}, async (t) => {
  const session = await createSession("ledger-conflict-persistence");
  const seed = [
