...
`server/orchestrator.js` selects providers from `server/providers/registry.js`. `server/run-state.js` gives each attempt a `runId` and absorbing terminal state; late provider results and old server-sent events are discarded once that attempt stops, fails, completes, or is replaced. A single provider failure no longer fences its siblings: they finish their round and the failed agent is retried once automatically (its aborted partial pruned so the recovery replaces it). A failure that survives that retry drops that agent from the rest of the session with a clear notice and the run continues on the remaining agents — one broken provider (e.g. a 401 from an un-logged-in CLI) never discards the answers the others gave. Only when too few agents remain to continue (debate needs both sides; every mode needs at least one) does the run persist one terminal error. A user cancellation still fences immediately. Startup reconciliation marks an orphaned stored run as interrupted exactly once.
